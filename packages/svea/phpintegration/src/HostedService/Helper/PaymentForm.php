...
class PaymentForm {
    ...
    public function setSubmitMessage($countryCode = FALSE) {
        switch ($countryCode) {
            case "SE":
                $this->submitMessage = "Kortbetalning (MasterCard eller Visa)";
                $this->noScriptMessage = "Javascript är inaktiverat i er webbläsare, så ni får manuellt omdirigera till paypage";
                break;
            default:
                $this->submitMessage = "Card payment with MasterCard Or Visa";
    ...
    public function setForm() {
        ...
        $formString .= "<button style='background-color:#fff; font-size: 1.4em; border:1px solid #aaa;' class='btn btn-block' type='submit' name='submit'><img height='50' class='pull-left' src='". asset('images/site/payment/SVEA_small.png') ."' alt='img'> 
            <span style='padding-top: 10px; padding-left: 10px' class='pull-left'> ".$this->submitMessage ."</span>";
        $formString .= "</button>";
        $formString .= "</form>";
