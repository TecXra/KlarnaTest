...
<div class="container main-container containerOffset">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12">
            ...
            <i class="fa fa-th-list" aria-hidden="true"></i> Du har valt att betala med Kort </span></h1>
        </div>
...
{{--<form action="{{ url('storeCardPayment')}}" method="POST">
        {{ csrf_field() }} --}}

    {{-- <form method="post" action="{{ url('/showDibsCardPayment') }}" accept-charset="utf-8"> --}}
    <form method="post" action="https://payment.architrade.com/paymentweb/start.action" accept-charset="utf-8">
        {{ csrf_field() }}

        <input type="hidden" name="accepturl" value="{{ url('/storeCardPayment') }}">
        <input type="hidden" name="amount" value="{{ $cartCalculator->totalPriceIncTax() * 100 }}">
        <input type="hidden" name="billingAddress" value="{{ Session::get('orderInfo.billingAddress') }}">
        <input type="hidden" name="billingFirstName" value="{{ Session::get('orderInfo.billingFirstName') }}">
        <input type="hidden" name="billingLastName" value="{{ Session::get('orderInfo.billingLastName') }}">
        <input type="hidden" name="billingPostalCode" value="{{ Session::get('orderInfo.billingPostalCode') }}">
        <input type="hidden" name="billingPostalPlace" value="{{ Session::get('orderInfo.billingCity') }}">
        <input type="hidden" name="cancelurl" value="{{ url('/kortbetalning') }}">
        <input type="hidden" name="currency" value="752">
        <input type="hidden" name="email" value="{{ Session::get('orderInfo.email') }}">
        <input type="hidden" name="lang" value="sv">
        <input type="hidden" name="merchant" value="{{ $dibsMerchantId }}">
        <input type="hidden" name="orderid" value="{{ Session::get('orderInfo.orderId') }}">
        <input type="hidden" name="paytype" value="VISA,MC">
        {{-- <input type="hidden" name="md5key" value="{{ Session::get('orderInfo.md5key') }}"> --}}
